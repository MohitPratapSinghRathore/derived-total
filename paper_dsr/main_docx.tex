...
detectable inside the observed range, a quadratic term of +0.0403
[+0.0194, +0.0599] that excludes zero and halves the residual, so a
single power law is a demonstrably wrong model of the total where the data actually
are. Extrapolated, the fitted parts reach 1.27 times the fitted whole
...
exponent itself, whose effective slope drifts from $-$0.299 to
$-$0.174, covering 27\% of the span of the category
...
\citep{rathore2026scale}{} decomposed the illegal
...
within 0.0060 in share and so present no further choice worth
...
reports it: one of the 5 categories is itself detectably curved
...
verdict is not. At one billion parameters the parts stand at 1.27 times
the whole [1.13, 1.57], above one in
100\% of resamples of the ladder: where the threshold falls is
...
18 runs gives a quadratic term of +0.0403, with a rung bootstrap
interval of [+0.0194, +0.0599] that excludes zero, and only
1.0\% of resamples on the other side. Treating runs as independent,
which is optimistic and which we report only because it is the conventional test,
gives $t = 8.61$ on 15 degrees of freedom. Adding the
quadratic term halves the residual, from 0.0280 to
0.0115, and is preferred by AIC.
...
12-fold span, with no extrapolation involved.
...
+0.0146 [+0.0087, +0.0257], against a
measured +0.0403. The total bends more than the decomposition can
...
+0.0257 [-0.0007, +0.0508], positive in
97\% of resamples but with an interval that includes zero by a
...
1 of 5 shows curvature whose interval
excludes zero: the check category, at +0.0379
[+0.0069, +0.0697]. That is the shallowest category and
...
range it moves by 0.071, and by one billion parameters it
...
\citep{amit2025loquacious} confirms the identity holds in 11 of
11 rows, to within what rounding four printed quantities to one decimal
...
supplier's, returns: largest class exponent $-$0.112 against an overall
exponent of $-$0.267, so the classes outgrow the total they belong to,
and a threshold of 174M parameters
[57M, 635M] past which the two reports
...
5 shows detectable log-log curvature, so the paper's condition
