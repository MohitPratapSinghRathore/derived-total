...
computable scale. On our own measurements the parts reach \result{cmpOverBn} times
the whole at one billion parameters and \result{cmpOverTn} times at one trillion.
The threshold sits at \result{cmpBreakdown} parameters, below the range our earlier
analysis had forecast into. An independent model family reproduces the defect at
\result{cmpExtBreakdown}. We contribute a diagnostic that needs only the exponents
a study already publishes, a closed remedy that fits the composition on the simplex
and is \result{cmpRmseGain}\% more accurate in range, and the finding that an
...
\ifanon (Anonymous, 2026)\else\citep{rathore2026coherent}\fi{} decomposed the
...
which it became so, \result{cmpBreakdown} parameters, lay well below the scale we
...
read. Our own values illustrate the mechanism: $a^\star$ is \result{cmpAstar}, the
rate-weighted mean of the part exponents is \result{cmpWeighted}, and the
separately fitted $b$ is \result{cmpBtotal}, giving a divergence rate of
\result{cmpDiverge}.
...
\result{cmpAstar} against a total exponent of \result{cmpBtotal}, a divergence rate
of \result{cmpDiverge}, and a breakdown scale of \result{cmpBreakdown} parameters at
...
stops being arithmetic, and the overspend there is \result{cmpOverBn}
[\result{cmpOverBnLo}, \result{cmpOverBnHi}], above one in
\result{cmpOverBnFrac}\% of resamples of the ladder.
...
\result{cmpBreakdown} parameters and say so. The second is to refit the composition
...
\result{cmpExtNclass} usable categories gives $a^\star$ \result{cmpExtAstar} against
a total of \result{cmpExtB}, a divergence rate of \result{cmpExtDiverge}, an
overspend of \result{cmpExtOverBn} at one billion parameters, and a breakdown scale
of \result{cmpExtBreakdown}. A different tokenisation, a different taxonomy, a
...
No, it improves it. Fitted in range across the \result{nRuns} runs, the closed model
attains a root-mean-square error of \result{cmpRmseSimplex} in share space against
\result{cmpRmseIndep} for the uncoupled fits, an improvement of
\result{cmpRmseGain}\%. This matters for adoption rather than for truth: an
...
\result{cmpCheckLarge} at the top of our range to \result{cmpCheckBn} at one billion
and \result{cmpCheckTn} at one trillion, approaching but never exceeding
\result{cmpCheckMax}, while the most local category falls to \result{cmpEmptyTn}.
...
make it observed. Every scale beyond \result{paramsLarge} is extrapolation, and a
...
already publishes. On our own measurements that scale was \result{cmpBreakdown}
parameters and we had read a forecast at one billion; an independently trained
family breaks down at \result{cmpExtBreakdown}. The remedy costs nothing, improving
in-range accuracy by \result{cmpRmseGain}\%, and the diagnostic costs a function
call.
...
described here is \texttt{code/closure\_check.py} and the closed model is
