...
bound. On our measurements the parts reach \result{admOverBn} times the fitted
whole at one billion parameters and \result{admOverTn} times at one trillion,
exceeding ten percent beyond \result{admBreakdown} parameters
[\result{admBreakdownLo}, \result{admBreakdownHi}]. The repair is to derive the
...
\result{admEffSmall} to \result{admEffBn}, covering \result{admDriftPct}\% of the
...
the parts there stand at \result{admOverBn} times the whole
...
\citep{loquacious2025supplementary} confirms it holds in \result{audClosed} of
\result{audRows} rows, with a worst discrepancy of \result{audWorst} points, which is
...
error analysis. Of the \result{audSources} sources we examined in detail,
\result{audWithAgg} fits the aggregate independently, and it is ours. We report this
