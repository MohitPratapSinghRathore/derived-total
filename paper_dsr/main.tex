...
It is worth being precise about where the error lies. A sum of power laws with
distinct exponents is not a power law, so a decomposition and a separately fitted
aggregate cannot both be correct specifications, and the aggregate is the term that
has to give way. This holds as a matter of arithmetic whatever the categories are
doing, which is what makes it a condition rather than an empirical claim.

We resist the stronger statement that the categories are each well specified and
only their total is not. On our data that is false, and Section~\ref{sec:eval}
reports it: one of the \result{curvNClasses} categories is itself detectably curved
in log-log, and it is the shallowest one, the category that dominates the total at
scale. The defensible claim is therefore the weaker one, in two parts. Even if every
category were an exact power law, the aggregate could not be. And in practice the
categories are not exact power laws either, which makes a single fitted aggregate
exponent worse rather than better than the condition alone implies.
...
\result{admLadderWide}-fold span, with no extrapolation involved.

\paragraph{Where that curvature comes from, and what it costs the argument.}
If every category were an exact power law, the curvature the aggregate must show
would be fixed by the category coefficients alone. That implied value is
\result{curvImplied} [\result{curvImpliedLo}, \result{curvImpliedHi}], against a
measured \result{curvMeasured}. The total bends more than the decomposition can
produce, and the measured value lies outside the implied interval.

We report the paired test rather than that comparison, because both quantities come
from the same eighteen runs. Refitting both on each rung resample, the gap is
\result{curvGap} [\result{curvGapLo}, \result{curvGapHi}], positive in
\result{curvGapPosPct}\% of resamples but with an interval that includes zero by a
narrow margin. On that test alone we could not claim the excess is established.

The category fits settle it. Fitting each category quadratically,
\result{curvNCurved} of \result{curvNClasses} shows curvature whose interval
excludes zero: the check category, at \result{curvCheckQ}
[\result{curvCheckLo}, \result{curvCheckHi}]. That is the shallowest category and
therefore the one that dominates the total at scale, so the aggregate inheriting
extra curvature from it is exactly what one would expect.

This costs the paper its stronger reading and we would rather say so than let a
reader find it. We cannot claim the categories are well specified and only their
total is not. What survives is the two-part claim of Section~\ref{sec:design}: the
arithmetic says that even exact category power laws cannot have a power-law total,
and the data say the categories are not exact power laws either. Both point the same
way, and the second makes a single fitted aggregate exponent a worse summary rather
than a better one. It also means the check category's own exponent, which our
companion study reports as the quantity that does not fall with scale, is itself a
local slope; that does not affect its sign or the ordering that study reports, but a
reader extrapolating it should know it bends.
...
\paragraph{A worked procurement check.}
The artifact is meant to be used by someone deciding whether to sign, so we set out
what that looks like with the numbers a buyer would actually hold. Suppose a
supplier of a document-processing service argues against funding a mitigation
programme on the grounds that the error class in question is on a steep curve. The
buyer asks for one thing: the fitted coefficients behind that claim, an exponent and
an intercept for each error class and for the overall error rate, together with the
range of model sizes they were fitted on. These are not proprietary; they are what
the supplier's own analyst already computed, and a supplier unwilling to provide
them has answered a different question.

Running the check on our own ladder's coefficients, which stand here for the
supplier's, returns: largest class exponent \result{admAstar} against an overall
exponent of \result{admBfitted}, so the classes outgrow the total they belong to,
and a threshold of \result{admBreakdown} parameters
[\result{admBreakdownLo}, \result{admBreakdownHi}] past which the two reports
contradict each other by more than a tenth. The buyer compares that threshold with
the scale the supplier's roadmap actually invokes. If the roadmap points at a model
ten times larger than the threshold, as ours did, the claim being made is not
supported by the analysis behind it, and the right response is not to reject the
supplier but to ask for the forecast restated up to the threshold, or restated with
the total derived rather than fitted. Both are cheap for the supplier to produce,
which is what makes the request reasonable to put in a contract schedule.

Two things make this usable rather than aspirational. It needs no access to weights,
data, or evaluation harness, so it survives a supplier's confidentiality position.
And it produces a dated artefact, the coefficients and the threshold, that an
internal auditor can read later without reconstructing anyone's reasoning.
...
\item[L2. The categories are not exact power laws either.] One of
\result{curvNClasses} shows detectable log-log curvature, so the paper's condition
is what survives rather than a claim that only the aggregate is misspecified. A
fuller treatment would model every level with a form that admits curvature, which
would change the coefficients while leaving the condition untouched.
\item[L3. Consistency is not validation.] The derived total is arithmetically
...
\item[L4. Three checkpoints in the independent family.] Its coefficients are
...
\item[L5. The invalidation claim is narrow.] What an inconsistent pair voids are
...
\item[L6. Depth and width are confounded.] Both ladders grow depth and width
together, so an exponent in parameters isolates neither.
\item[L7. Partitions only.] The condition is derived for categories that partition a
total. Overlapping categories are common in practice and need a different bound,
which we have not derived.
\item[L8. The audit is an existence check, not a survey.] We establish that speech
...
\item[L9. The tolerance is a convention.] Ten percent is a defensible default and
...
\item[FR2] Refit every level of the decomposition with a form that admits
curvature, such as a broken or multi-term law, and establish whether the consistency
condition has an analogue once neither the parts nor the whole are power laws, which
we expect it does but have not derived.
\item[FR3] Compare the estimation losses rather than the links, since the derived
...
\item[FR4] Extend the independent replication to a family with more checkpoints, so
that consistency can be tested against exponents estimated with usable precision
rather than only in direction.
\item[FR5] Characterise precisely which inferences an inconsistent pair voids, by
...
\item[FR6] Separate depth from width along a ladder that varies them independently,
and determine whether the divergence rate is a property of parameter count or of
one of its components.
\item[FR7] Derive the corresponding bound for overlapping categories, which would
extend the check to severity scales and multi-label failure taxonomies as used in
industrial evaluation.
\item[FR8] Conduct a survey of published per-category scaling forecasts, applying
...
\item[FR9] Study the tolerance itself, relating it to the sampling error of the
...
\url{https://github.com/MohitPratapSinghRathore/derived-total}. The diagnostic
