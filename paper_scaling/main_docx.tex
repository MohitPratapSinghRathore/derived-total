...
18 of 18 models, because hedging lowers it. Finally,
per-category scaling fits are not closed: our own extrapolated shares sum to
1.34 at one billion parameters. Fitting the composition on the simplex
...
shares gives 1.02 at our largest rung, which is fit noise, but
1.34 at one billion parameters and 3.93 at one trillion,
where the single largest category is assigned 2.65 of the failures.
...
attains RMSE 0.01073 across the 18 runs against
0.01148 for the uncoupled fits, so it is at least as good on the
...
450M parameters
(77M to 2.9B, 99.8\% of bootstrap
resamples above our largest model) and a share of 0.362, and then
declines, because it is in turn displaced by the category that never scales away.
Failures despite correctly decoded local belief overtake it at 611M
parameters and reach 0.674 of failures at one trillion, while the
locally decidable categories fall to 0.000. The paper's qualitative
...
99.8\% of resamples. The closed fit guarantees that the
