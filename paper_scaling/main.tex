...
\result{invCount} of \result{invN} models, because hedging lowers it. Finally,
per-category scaling fits are not closed: our own extrapolated shares sum to
\result{cmpSumBn} at one billion parameters. Fitting the composition on the simplex
fits better in range, and moves the projection from an impossible one to a peak.
\end{abstract}
...
sum to the illegal-move rate at every scale. Six independently fitted power laws
with six different exponents cannot satisfy that identity away from the fitted
range: the shallowest exponent must eventually dominate a total that is itself
falling faster. The failure is not gradual. Summing the independently fitted
shares gives \result{cmpSumLarge} at our largest rung, which is fit noise, but
\result{cmpSumBn} at one billion parameters and \result{cmpSumTn} at one trillion,
where the single largest category is assigned \result{cmpMaxTn} of the failures.
A category holding more than all of them is not a wide estimate, it is an
...
time. We model the shares with a softmax linear in $\log N$, which is closed by
construction: the shares sum to one at every scale and for every parameter value,
so a category can never be assigned more failures than occur. The illegal-move
rate keeps its own power law, which is legitimate because the total is not a part
of anything. Coherence is not bought with fit quality. In range the closed model
...
The correction changes the projection qualitatively, and in a direction we did not
anticipate. Under uncoupled fits the check category rises without limit and appears
to absorb nearly all failures. Under the closed model it peaks, at
\result{cmpPeak} parameters
(\result{cmpPeakLo} to \result{cmpPeakHi}, \result{cmpPeakAbove}\% of bootstrap
resamples above our largest model) and a share of \result{cmpPeakShare}, and then
declines, because it is in turn displaced by the category that never scales away.
Failures despite correctly decoded local belief overtake it at \result{cmpOvertake}
parameters and reach \result{cmpPolicyTn} of failures at one trillion, while the
locally decidable categories fall to \result{cmpEmptyTn}. The paper's qualitative
claim is unchanged, and the ordering is if anything sharper: what scale removes
first is local error, what it removes last is not global error but error that
survives a correct local belief. We stress that the peak lies outside our fitted
range and is a property of the model, not an observation. The difference from the
earlier version is that it is now a coherent implication of the exponents rather
...
fits are not closed, and extending them makes claims the categories cannot jointly
satisfy. The check costs nothing: sum the extrapolated parts and see whether they
...
\item[The peak is a model property, not a measurement.] The scale at which the
check category turns over lies above our largest model in
\result{cmpPeakAbove}\% of resamples. The closed fit guarantees that the
projection is arithmetically possible, which the uncoupled fits do not; it does
not make it observed, and a different link function would move the turn.
