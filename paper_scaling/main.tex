...
per-category scaling fits are not closed. Whenever the exponents differ, the parts
must outgrow the whole beyond a computable scale; ours exceed it by
\result{cmpOverBn}-fold at one billion parameters, and Karvonen's ladder reproduces
the defect. Fitting the composition on the simplex is closed by construction and
fits better in range.
\end{abstract}
...
\item[The closed fit is coherent, not validated.] The softmax link guarantees the
