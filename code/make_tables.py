...
def strictness_table():
    ...
    for lo, hi in d["buckets"]:
        ...
        for t in tols:
            ...
            if not c or c["median_cp"] is None or c["n"] < 30:
                cells.append("n/a")
            else:
...
def stats_table():
    ...
    L = ["\\begin{table}[t]", "\\centering\\small",
         ...
         "\\label{tab:stats}", "\\begin{tabular}{lcc}", "\\toprule",
         "Quantity & Estimate & 95\\% CI (clustered) \\\\", "\\midrule"]
    for k, lab in (("belief", "belief consistency"),
                   ("belief_mismatched", "\\quad mismatched-belief control"),
                   ("belief_random", "\\quad random-move control"),
                   ("auc_aggregate", "AUC, whole board"),
                   ("auc_touched", "AUC, move-touched")):
        v = d.get(k)
        if not v:
            continue
        est = v.get("mean", v.get("auc"))
        L.append(f"{lab} & {est:.4f} & [{v['ci_lo']:.4f}, {v['ci_hi']:.4f}] \\\\")
    ad = d.get("auc_difference")
    if ad:
        L.append(f"\\quad difference & {ad['diff']:+.4f} & "
                 f"[{ad['ci_lo']:+.4f}, {ad['ci_hi']:+.4f}] \\\\")
    h = d.get("heldout_logloss", {})
    if h:
        L += ["\\midrule", "Held-out log loss & \\multicolumn{2}{c}{} \\\\"]
        for k, lab in (("depth_only", "depth only"),
                       ("plus_aggregate", "\\quad $+$ whole board"),
                       ("plus_touched", "\\quad $+$ move-touched"),
                       ("plus_both", "\\quad $+$ both"),
                       ("plus_both_and_confidence", "\\quad $+$ both and confidence")):
            if isinstance(h.get(k), float):
                L.append(f"{lab} & {h[k]:.4f} & \\\\")
    L += ["\\bottomrule", "\\end{tabular}", "\\end{table}", ""]
    return L


def cycles_table():
    """Identical position, differing transcript length."""
    d = load("attn_8L256_s0_cycles")
    if not d:
        return []
    L = ["\\begin{table}[t]", "\\centering\\small",
         ...
         "\\label{tab:cycles}", "\\begin{tabular}{lccc}", "\\toprule",
         "Added plies & occupied-square acc. & 95\\% CI & illegal rate \\\\",
         "\\midrule"]
    for k in sorted(d["per_cycle"], key=int):
        v = d["per_cycle"][k]
        L.append(f"{k} & {v['occ_acc']:.3f} & [{v['ci_lo']:.3f}, {v['ci_hi']:.3f}] "
                 f"& {v['illegal']:.3f} \\\\")
    L += ["\\bottomrule", "\\end{tabular}", "\\end{table}", ""]
    return L
...
def sqcontrol_table():
    """Which squares, not how many: random-pair controls with clustered CIs."""
    d = load("attn_8L256_s0_sqcontrol")
    if not d:
        return []
    order = [("whole_board", "whole board, 64 squares"),
             ("two_random", "two random squares"),
             ("two_random_occupied", "two random occupied squares"),
             ("source_only", "origin square only"),
             ("destination_only", "destination square only"),
             ("move_touched", "move-touched, origin and destination")]
    L = ["\\begin{table}[t]", "\\centering\\small",
