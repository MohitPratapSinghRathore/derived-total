...
# ----------------------------- transfer edit, random play, reversible cycles
_tr = load("attn_8L256_s0_transfer")
if _tr:
    _src = "results/attn_8L256_s0_transfer.json"
    put("trFlipA", _tr["manipulation"]["flip_A_to_empty"], _src)
    put("trFlipB", _tr["manipulation"]["flip_B_to_piece"], _src)
    put("trN", _tr["n"], _src, "{:.0f}")
    put("trBothN", _tr.get("n_both_flipped"), _src, "{:.0f}")
    for _k, _nm in (("outA", "trOutA"), ("outB", "trOutB"),
                    ("outA_rand", "trOutARand"), ("outB_rand", "trOutBRand")):
        _v = _tr.get(_k)
        if _v:
            put(_nm, _v["mean_delta"], _src, "{:+.4f}")
            put(_nm + "Lo", _v["ci_lo"], _src, "{:+.4f}")
            put(_nm + "Hi", _v["ci_hi"], _src, "{:+.4f}")
            _b = _v.get("both_flipped")
            if _b:
                put(_nm + "Both", _b["mean_delta"], _src, "{:+.4f}")
                put(_nm + "BothLo", _b["ci_lo"], _src, "{:+.4f}")
                put(_nm + "BothHi", _b["ci_hi"], _src, "{:+.4f}")

_rp = load("attn_8L256_s0_randomplay")
if _rp:
    _src = "results/attn_8L256_s0_randomplay.json"
    put("rpBelief", _rp["belief_consistency"], _src)
    put("rpRand", _rp["random_move_control"], _src)
    put("rpBelN", _rp["belief_n"], _src, "{:,.0f}")
    put("rpOccFirst", _rp["occ_acc"][0], _src)
    put("rpOccLast", _rp["occ_acc"][-1], _src)
    put("rpIllFirst", _rp["illegal"][0], _src)
    put("rpIllLast", _rp["illegal"][-1], _src)
    put("rpGames", _rp["n_games"], _src, "{:,.0f}")

_cy = load("attn_8L256_s0_cycles")
if _cy:
    _src = "results/attn_8L256_s0_cycles.json"
    put("cycN", _cy["n"], _src, "{:.0f}")
    for _k, _v in _cy["per_cycle"].items():
        put(f"cycAcc{_k}", _v["occ_acc"], _src)
        put(f"cycLo{_k}", _v["ci_lo"], _src)
        put(f"cycHi{_k}", _v["ci_hi"], _src)
        put(f"cycIll{_k}", _v["illegal"], _src)



# ----------------------------------- clustered statistics and held-out models
_st = load("attn_8L256_s0_stats")
if _st:
    _src = "results/attn_8L256_s0_stats.json"
    put("stN", _st["n"], _src, "{:,.0f}")
    put("stGames", _st["n_games"], _src, "{:,.0f}")
    put("stIll", _st["n_illegal"], _src, "{:,.0f}")
    for _k, _nm in (("belief", "cbBel"), ("belief_mismatched", "cbBelMis"),
                    ("belief_random", "cbBelRand")):
        _v = _st[_k]
        put(_nm, _v["mean"], _src)
        put(_nm + "Lo", _v["ci_lo"], _src)
        put(_nm + "Hi", _v["ci_hi"], _src)
    for _k, _nm in (("auc_aggregate", "cbAucAgg"), ("auc_touched", "cbAucTouch")):
        _v = _st[_k]
        put(_nm, _v["auc"], _src)
        put(_nm + "Lo", _v["ci_lo"], _src)
        put(_nm + "Hi", _v["ci_hi"], _src)
    _d = _st["auc_difference"]
    put("cbAucDiff", _d["diff"], _src, "{:+.4f}")
    put("cbAucDiffLo", _d["ci_lo"], _src, "{:+.4f}")
    put("cbAucDiffHi", _d["ci_hi"], _src, "{:+.4f}")
    _g = _st.get("gee", {})
    for _n, _nm in (("aggregate", "geeAgg"), ("move_touched", "geeTouch"),
                    ("low_confidence", "geeConf"), ("depth", "geeDepth")):
        if _n in _g and isinstance(_g[_n], dict):
            put(_nm, _g[_n]["coef"], _src, "{:+.3f}")
            put(_nm + "SE", _g[_n]["robust_se"], _src)
    _h = _st["heldout_logloss"]
    for _k in ("depth_only", "plus_aggregate", "plus_touched", "plus_both",
               "plus_both_and_confidence"):
        put("ll" + _k.replace("_", ""), _h[_k], _src, "{:.4f}")
    _a = _st["abstain_heldout"]
    for _c in ("0.5", "0.7"):
        _tag = _c.replace(".", "")
        for _k, _nm in (("model_confidence", "hoConf"), ("probe_certainty", "hoCert"),
                        ("combined_deployable", "hoComb"), ("oracle_bound", "hoBound")):
            put(f"{_nm}{_tag}", _a[_c][_k], _src)
    put("hoBase", _a["1.0"]["model_confidence"], _src)



# ------------------- clustered intervals on the PRIMARY belief population
_bc = load("attn_8L256_s0_belief")
if _bc and "clustered" in _bc:
    _c = _bc["clustered"]; _src = "results/attn_8L256_s0_belief.json"
    for _k, _nm in (("own", "pcBel"), ("mis", "pcBelMis"), ("rnd", "pcBelRand")):
        put(_nm, _c[_k]["mean"], _src)
        put(_nm + "Lo", _c[_k]["ci_lo"], _src)
        put(_nm + "Hi", _c[_k]["ci_hi"], _src)
    put("pcGames", _c["n_games"], _src, "{:,.0f}")
    put("pcN", _c["n"], _src, "{:,.0f}")



# --------------------------------------------- depth-local probes and drift
_dr = load("attn_8L256_s0_drift")
if _dr:
    _src = "results/attn_8L256_s0_drift.json"
    _d = _dr["within_depth"]; _g = _dr["global_probe"]
    put("drDiagFirst", _d[0], _src); put("drDiagLast", _d[-1], _src)
    put("drPoolFirst", _g[0], _src); put("drPoolLast", _g[-1], _src)
    _M = _dr["transfer"]
    put("drFarOff", _M[0][-1], _src)      # shallow probe on deep positions
