...
it move-touched, because it is a deliberately local proxy. The state an action
...
the time, so the believed board is not permissive. For reference, genuinely legal moves are recovered as legal in the believed board
...
model's decoding ceiling, so the honest quantity is the ratio of the two.
...
\result{beliefMisBoxAttn}. What we have characterised strengthens with scale over
...
is a weak guide to what a model will do next. The same measurement, restricted to
