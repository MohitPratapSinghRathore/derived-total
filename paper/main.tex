...
Aggregate state fidelity, the quantity this literature reports, barely predicts
behaviour: within a fixed depth the correlation between misremembered squares and
playing an illegal move lies between \result{corrGlobalMinAttn} and
\result{corrGlobalMaxAttn}. Restricted to the squares a move touches, it predicts
strongly, \result{locIllAttn} against \result{locLegAttn}. And the errors are
...
\result{belMisAttn} for the same move under another position's belief at equal
depth and \result{belRandAttn} for an arbitrary illegal move. Editing the probe
direction encoding an occupied square removes \result{patchCutAttn} of the
probability mass on moves from it, a norm-matched random edit
\result{patchRandCutAttn}, so the representation is read and not merely present.
The effect strengthens across a twelvefold range of model size. The model is not
emitting noise; it plays correctly on a board it has misremembered.

We report failures too: a predicted layer profile that ran backwards, a
degenerate pre-registered metric, an auxiliary objective that improves the
measurement of state more than its consequences, and a second domain needing
seven designs to become measurable.
\end{abstract}
